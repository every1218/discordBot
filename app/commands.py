...
async def handle_simple_commands(message, today):
    if message.content == ".도움말":
        embed = discord.Embed(title="🐧 펭귄봇 명령어 목록", color=0x3498db)
        ...
        economy_text = (
            ...
            "`.송금 [금액] [@유저]` : 다른 유저에게 돈을 보냅니다.\nㅤ"
        )
        embed.add_field(name="💰 경제 명령어", value=economy_text, inline=False)
        
        # 게임 명령어
        games_text = (
            "`.홀짝 [홀/짝] [금액]` : 주사위 홀짝 게임입니다.\n"
            "`.잭팟` : 슬롯머신을 돌려 잭팟을 노립니다. (베팅금 10000원)\n"
            "`.복권 [숫자1]..[숫자6]` : 1~20 숫자 6개를 맞춥니다. (베팅금 5000원)\n"
            "`.블랙잭 [금액]` : 딜러와 블랙잭 게임을 합니다.\nㅤ"
        )
        embed.add_field(name="🎮 게임 명령어", value=games_text, inline=False)
        
        # 롤 카운터 & AI 명령어
        others_text = (
            "`.카운터 [챔피언 이름]` : 롤 챔피언의 카운터 정보를 보여줍니다.\n"
            "`@봇이름 [할 말]` : 봇을 멘션하여 인공지능과 대화할 수 있습니다."
        )
        embed.add_field(name="⚔️ 롤 카운터 & AI 명령어", value=others_text, inline=False)
